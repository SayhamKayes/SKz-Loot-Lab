...
function AdminPage() {
  ...
  // Active admin tab
  const [tab, setTab] = useState<"orders" | "games" | "settings">("orders");
  ...
  const handleDeleteGame = async (gameId: number) => {
    if (!confirm("Are you sure you want to delete this game and all its packages?")) return;
    try {
      await adminDeleteGameFn({ data: { id: gameId } });
      loadGames();
    } catch (e) {
      console.error(e);
    }
  ...
  const handleDeletePackage = async (packageDbId: number) => {
    if (!confirm("Delete this coin package?")) return;
    try {
      await adminDeletePackageFn({ data: { id: packageDbId } });
      loadGames();
    } catch (e) {
      console.error(e);
    }
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-800 bg-slate-900/90 backdrop-blur-xl">
        ...
        <div className="border-t border-slate-800/80 bg-slate-900/60 px-4 sm:px-6">
          <div className="mx-auto max-w-7xl flex gap-6 text-sm font-semibold">
            <button
                      ...
                      {g.packages?.map((p: any) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {p.dbId && (
                              <button
                                onClick={() => handleDeletePackage(p.dbId)}
